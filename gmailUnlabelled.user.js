...
if (edit_labels) {
	...
	label_none.setAttribute ("onclick", "var iter = this; var str = ''; while (iter.previousSibling != null) { iter = iter.previousSibling; str = str + ' -label:' + iter.getAttribute('id').substr(3); }var s = document.getElementById ('s'); for (var i = 0; i < s.childNodes.length; i++) {	var q = s.childNodes[i]; 	if (q.getAttribute('name') == 'q') 	{		q.value = str; 		return top.js._MH_OnSearch(window,0);	}}");
